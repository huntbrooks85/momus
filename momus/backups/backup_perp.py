...
class PolynomialModel(object):
    ...
    def legendre_second_derivative(self, x, coeffs):
        ...
        scale = 2.0 / (self.xmax - self.xmin)
        return scale**2 * np.polynomial.legendre.legval(self.scale_x(x), d2coeff)
    
    def project_x0(self, coeffs):
        x0 = np.empty_like(self.x, dtype=float)
        P = np.polynomial.legendre.Legendre(coeffs).convert(kind=np.polynomial.Polynomial)
        P1 = P.deriv(1)
        P2 = P.deriv(2)
        ...
        for i, (xi, yi) in enumerate(zip(self.x, self.y)):
            ...
            p2 = P2(xi)
            r = yi - p
            a = 0.5 * (r * P.deriv(3)(xi) - 3.0 * p1 * p2)
            b = r * p2 - 1.0 - p1**2
            c = r * p1
            roots = np.roots([a, b, c])
            roots = roots[np.abs(roots.imag) < 1e-8].real
            d = roots[np.argmin(np.abs(roots))] if len(roots) else 0.0
            x0[i] = xi + d
    ...
    def ln_likelihood(self, pars):
        ...
        # g = (self.x - x0 + (self.y - P0) * dP0)

        # print("\n--- Projection diagnostic ---")
        # print("max |g|:", np.max(np.abs(g)))
        # print("median |g|:", np.median(np.abs(g)))
...
def hoggfit(x, y, xerr=None, yerr=None, xyerr=None,
    ...
    y0 = model.legendre_model(x, coeffs0)
    ...
    print("coeffs0:", coeffs0)

    print("sigma0:", sigma0)

    print(
        "RMSE initial:",
        np.sqrt(np.mean((y - y0)**2))
    )
    ...
    print(
        ...
        model.ln_posterior(initial_params_test)
    )
    ...
    for k in range(order + 1):
        ...
        initial_params[:, -1] = (sigma0 * np.abs(1.0 + 0.05 * np.random.randn(nwalkers)))
